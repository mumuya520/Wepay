...
class Swiftpass2Plugin : public ChannelPlugin {
public:
    std::string name() const override { return "swiftpass2"; }
    ...
    ChannelOrderResult createOrder(const ChannelOrderRequest &req) override {
        ...
        if (p.get("appid", "").asString().empty() || p.get("appkey", "").asString().empty()) {
            r.errMsg = "威富通MD5参数不完整(appid/appkey)";
            return r;
        }
...
private:
    ...
    // MD5 uppercase sign: ksort, k=v& (skip sign/empty/trim-empty), remove last &, append &key=xxx
...
REGISTER_CHANNEL_PLUGIN(Swiftpass2Plugin);
