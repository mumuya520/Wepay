...
// AgPay Plus 上游通道插件
// 协议与 Jeepay 完全相同，reqTime 使用 Unix 秒（AgPay 官方 SDK 行为）
class AgpayPlusPlugin : public ChannelPlugin {
public:
    std::string name() const override { return "agpayplus"; }
    ...
    Json::Value paramSchema() const override {
        ...
        add("appmchid","商户号 mchNo","input");
        add("appid",   "应用 AppId", "input");
        add("appkey",  "私钥 ApiKey","textarea");
        add("apptype", "支付模式",   "input", "1",
            "1扫码 2H5/PC 3JSAPI/WAP 4小程序 5聚合扫码 6WEB收银台 7APP，逗号分隔");
        return arr;
    }
    ...
    ChannelOrderResult createOrder(const ChannelOrderRequest &req) override {
        ...
        if (appurl.empty() || mchNo.empty() || appId.empty() || key.empty()) {
            result.errMsg = "AgpayPlus 参数不完整(appurl/appmchid/appid/appkey)";
            return result;
        }

        std::string wayCode = chooseWayCode(req.payType, p.get("apptype", "1").asString());
        if (wayCode.empty()) {
            result.errMsg = "AgpayPlus 未找到可用支付模式(apptype)";
            return result;
        }
        ...
        params["reqTime"]    = std::to_string(nowSecs());  // AgPay 使用秒级时间戳
        params["version"]    = "1.0";
        params["signType"]   = "MD5";
        ...
        if (!resp.success) {
            result.errMsg = "AgpayPlus 请求失败: " + resp.errMsg;
            return result;
        }

        Json::Value j;
        if (!Json::Reader().parse(resp.body, j)) {
            result.errMsg = "AgpayPlus 响应解析失败";
            return result;
        }
        if (j.get("code", -1).asInt() != 0) {
            result.errMsg = j.get("msg", "AgpayPlus 下单失败").asString();
            return result;
        }
        ...
        if (payData.empty()) {
            result.errMsg = "AgpayPlus 未返回 payData";
            return result;
        }
    ...
    ChannelRefundResult refund(const ChannelRefundRequest &req) override {
        ...
        params["reqTime"]      = std::to_string(nowSecs());
        params["version"]      = "1.0";
        params["signType"]     = "MD5";
        params["sign"]         = makeSign(params, key);
        ...
        if (!Json::Reader().parse(resp.body, j) || j.get("code", -1).asInt() != 0) {
            result.errMsg = j.get("msg", "AgpayPlus 退款失败").asString();
            return result;
        }
        Json::Value data = j["data"];
        std::string errMsg = data.get("errMsg", "").asString();
...
private:
    ...
    static std::string chooseWayCode(const std::string &payType, const std::string &apptype) {
        auto has = [&](const std::string &x) {
            return ("," + apptype + ",").find("," + x + ",") != std::string::npos;
        };
    ...
    // AgPay reqTime 使用 Unix 秒（与 Jeepay 毫秒不同）
...
REGISTER_CHANNEL_PLUGIN(AgpayPlusPlugin);
