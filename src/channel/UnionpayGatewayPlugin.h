...
// 银联前置(unionpay_gateway) - MD5大写签名, XML POST/响应 (威富通协议)
// 网关: https://qra.95516.com/pay/gateway (可自定义)
// 签名: ksort拼接k=v&(skip sign/空值/trim空), 去尾&, 追加&key=xxx, MD5 uppercase
// 请求: mch_id/version/sign_type/nonce_str/service/.../sign → XML POST
// 响应: XML, status=0, result_code=0
// 扫码统一: unified.trade.native
// 微信JSAPI: pay.weixin.jspay (is_raw=1, is_minipg=0|1)
// 微信H5: pay.weixin.wappay
// 支付宝服务窗: pay.alipay.jspay (buyer_id)
// 云闪付JS: pay.unionpay.jspay (user_id)

class UnionpayGatewayPlugin : public ChannelPlugin {
public:
    std::string name() const override { return "unionpay_gateway"; }
    ...
    Json::Value paramSchema() const override {
        ...
        add("appurl", "自定义网关URL", "input", "", "可不填,默认https://qra.95516.com/pay/gateway");
        add("apptype", "支付方式", "select", "1", "1=扫码 2=JSAPI/公众号 3=H5 4=支付宝服务窗 5=云闪付JS");
        return arr;
    }
    ...
    ChannelOrderResult createOrder(const ChannelOrderRequest &req) override {
        ...
        if (p.get("appid", "").asString().empty() || p.get("appkey", "").asString().empty()) {
            r.errMsg = "银联前置参数不完整(appid/appkey)";
            return r;
        }
        ...
        if (service == "pay.alipay.jspay") {
            m["buyer_id"] = p.get("openid", p.get("sub_openid", "").asString()).asString();
        }
        if (service == "pay.unionpay.jspay") {
            m["user_id"] = p.get("openid", p.get("sub_openid", "").asString()).asString();
        }

        auto result = requestApi(p, m, key);
        r.rawResponse = result.rawResponse;
        if (!result.success) { r.errMsg = result.errMsg; return r; }

        r.success = true;
        if (service == "pay.weixin.jspay" || service == "pay.weixin.wappay" || service == "pay.alipay.jspay") {
            r.payUrl = get(result.data, "pay_info");
        } else if (service == "pay.unionpay.jspay") {
            r.payUrl = get(result.data, "pay_url");
        } else {
            r.payUrl = get(result.data, "code_url");
        }
...
private:
    ...
    static ApiResponse requestApi(const Json::Value &p, const std::map<std::string, std::string> &bizParams, const std::string &key) {
        ApiResponse ar;
        std::string mchId = p.get("appid", "").asString();
        std::string gatewayUrl = p.get("appurl", "").asString();
        if (gatewayUrl.empty()) gatewayUrl = "https://qra.95516.com/pay/gateway";
        ...
        if (signIt != result.end() && !signIt->second.empty()) {
            std::string calcSign = makeSign(result, key);
            if (calcSign != signIt->second) {
    ...
    static std::string getService(const std::string &payType, int apptype) {
        ...
        if (payType.find("alipay") != std::string::npos) {
            if (apptype == 4) return "pay.alipay.jspay";
            return "unified.trade.native";
        }
        if (payType == "bank" || payType == "bank_jsapi") {
            if (apptype == 5) return "pay.unionpay.jspay";
            return "unified.trade.native";
        }
        return "unified.trade.native";
    }
...
REGISTER_CHANNEL_PLUGIN(UnionpayGatewayPlugin);
